...
* TODO Write an elisp command to send org src blocks to a running vterm
buffer instead of evaluating via org-babel. This would handle multi-line
commands, interactive prompts (sudo, etc.), and persistent shell state.
For now, copy-paste blocks into a terminal.
...
cp ~/dotfiles/systemd/emacs.service.d/override.conf ~/.config/systemd/user/emacs.service.d/override.conf
