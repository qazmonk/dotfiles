...
Run this in a terminal (not org-babel — needs interactive password):
: chsh -s /bin/bash
** Install starship
#+begin_src bash
...
#+end_src
* Setup emacs daemon
Start emacs as a daemon via XDG autostart so it launches after the full graphical
session is ready (with =DISPLAY=, =WAYLAND_DISPLAY=, etc. properly set).
#+begin_src bash
mkdir -p ~/.config/autostart
ln -sf ~/dotfiles/applications/emacs-daemon.desktop ~/.config/autostart/emacs-daemon.desktop
#+end_src

If migrating from the old systemd approach, disable and clean up the service:
#+begin_src bash
systemctl --user disable --now emacs
rm -f ~/.config/systemd/user/emacs.service.d/override.conf
#+end_src

#+RESULTS:

* Setup LanguageTool
Download the standalone zip from https://languagetool.org and unzip it to =~/languagetool=. Then install Java and enable the systemd service:
#+begin_src bash
sudo dnf install java-latest-openjdk
ln -s ~/dotfiles/systemd/languagetool.service ~/.config/systemd/user/
